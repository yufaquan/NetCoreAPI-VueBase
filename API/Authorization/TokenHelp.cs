...
namespace Authorization
{
    public class TokenHelp
    {
        ...
        /// <param name="userId">用户Id</param>
        /// <param name="jmStr">UserToken</param>
        /// <param name="hours">延长的小时</param>
        public static async Task ExtensionTimeToHoursAsync(int userId, string jmStr,int hours=30)
        {
            await Task.Run(() => {
                //若存在 则覆盖
                ICacheManager cache = CacheService.GetCacheManager();
                ...
                cache.Set(cacheKey, jmStr, TimeSpan.FromHours(hours), TimeSpan.FromHours(hours));
            });
        }
        ...
        {
            string key = new { id = userId, key = Config.UserTokenSY,platform = Current.VisitToKey}.ToJsonString();
            //加密
