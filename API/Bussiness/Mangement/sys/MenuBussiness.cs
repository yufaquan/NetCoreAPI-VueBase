...
namespace Bussiness.Mangement
{
    public class MenusBussiness
    {
        ...
        public List<Menus> GetHavePermissionsList(bool isRoute=false)
        {
            ...
            {
                if (isRoute)
                    sjklist = ServiceHelp.GetMenuService.GetAllList(x => x.PId == 0 && !x.Hide).OrderBy(x=>x.Sort).ToList();
                else
                    sjklist = ServiceHelp.GetMenuService.GetAllList(x =>!x.Hide).ToList();
            }
            else if(allP !=null && allP.Count>0)
            {
                if (isRoute)
                    sjklist = ServiceHelp.GetMenuService.GetAllList(x => x.PId== 0 && !x.Hide && allP.IsHaveSame(x.Permissions.ToList(','))).OrderBy(x => x.Sort).ToList();
                else
                    sjklist = ServiceHelp.GetMenuService.GetAllList(x =>!x.Hide && allP.IsHaveSame(x.Permissions.ToList(','))).ToList();
            }
            return sjklist;
        }
        ...
        /// 递归获取子菜单
        /// </summary>
        /// <param name="isHide">是否隐藏禁用的菜单</param>
        /// <param name="list"></param>
        private void GetChildren(bool isHide,ref List<Menus> list)
        {
            ...
            foreach (var item in list)
            {
                ...
                if (isHide)
                {
                    clist = _service.GetAllList(x => x.PId == item.Id && !x.Hide).OrderBy(x => x.Sort).ToList();
                }
                else
                {
                    clist = _service.GetAllList(x => x.PId == item.Id ).OrderBy(x => x.Sort).ToList();
                }
                GetChildren(isHide,ref clist);
                item.Children = clist;
            }
        }
        ...
        public Menus Edit(Menus menus, out string errorMessage)
        {
            ...
            var data = _service.GetById(menus.Id);
            if (data == null)
            {
                errorMessage = "未查询到数据！";
                return null;
