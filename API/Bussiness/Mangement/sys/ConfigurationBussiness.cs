   ...
   public class ConfigurationBussiness
    {
        ...
        public Configuration Get(HttpContext httpContext)
        {
            ...
            foreach (PropertyInfo p in t.GetProperties())
            {
                if (data.ContainsKey(p.Name))
                {
                    ...
                    {
                        if (data[p.Name].ToString()=="1")
                        {
                            p.SetValue(result, true);
                        }
                        else if (data[p.Name].ToString() == "0")
                        {
        ...
        public bool Set(Configuration configuration, HttpContext httpContext)
        {
            ...
            try
            {
                ...
                log.Content = $"用户\"{log.UserName}\"修改了[{typeof(Configuration).GetDisplayName()}]数据。";
                log.EventType =  Enums.EventType.Update;
                log.Remark = "修改后数据："+configuration.ToJsonString();
                log.WriteDate = DateTime.Now;
                Task task = ServiceHelp.GetLogService.WriteEventLogAsync(log);
                return true;
            }
            catch (Exception ex)
            {
